...
public:
    ...
    : m_Position(sparky::maths::vec3(x, y, 0.0f)), m_Size(sprite->getSize()), m_Sprite(sprite), m_Collision(collision)
    {
        
    }
    
    LevelObject(float x, float y, float width, float height, sparky::graphics::Sprite* sprite, bool collision = true)
    : m_Position(sparky::maths::vec3(x, y, 0.0f)), m_Size(width, height), m_Sprite(sprite), m_Collision(collision)
    {
    ...
    inline sparky::maths::vec2 getCenter() { return sparky::maths::vec2(m_Position.x + m_Size.x / 2.0f, m_Position.y + m_Size.y / 2.0f);}
